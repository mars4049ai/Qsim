"""
qsim.py -- Модуль квантового распределения ключей и квантовой памяти.

Содержит классы:
    Qsim               -- симуляция протокола BB84, шифрование сообщений, демонстрация MITM-атаки.
    QuantumMemoryServer -- сервер хранения данных на базе квантовых осцилляторов (кудиты).
"""
...
class Qsim:
    """
    Симулятор квантового распределения ключей (QKD) на основе протокола BB84.

    Реализует полный цикл квантовой коммуникации:
        - генерацию квантового ключа между двумя сторонами (Алиса и Боб),
        - обнаружение присутствия перехватчика (Ева) по уровню QBER,
        - шифрование и расшифрование сообщений на основе квантового ключа (XOR + SHA-256),
        - демонстрацию классического обмена сообщениями через защищённый канал,
    ...
    Параметры:
        ...
        use_quantum_memory (bool) : включить квантовую память (QuantumMemoryServer) в демонстрацию.
        default_eve_active (bool) : активировать перехватчика по умолчанию при первом QKD-сеансе.
    """
    ...
    def __init__(self, num_bits=32, use_quantum_memory=False, default_eve_active=False):
        ...
        self.alice_name = 1  # переопределяется в run_demonstration
        self.bob_name = 2
        self.eve_name = 3
    ...
    def _xor_bytes(self, data_bytes, key_hash):
        """Побайтовое XOR-шифрование данных с ключом."""
    ...
    def encrypt_message(self, plaintext_message, quantum_key_str):
        """
        Зашифровать текстовое сообщение квантовым ключом.

        Алгоритм: XOR с SHA-256-хешем ключа, результат кодируется в Base64.

        Параметры:
            plaintext_message (str) : открытое текстовое сообщение.
            quantum_key_str (str)   : строковое представление квантового ключа.

        Возвращает:
            str : зашифрованное сообщение в формате Base64.
        """
    ...
    def decrypt_message(self, base64_encrypted_message, quantum_key_str):
        """
        Расшифровать Base64-сообщение квантовым ключом.

        Параметры:
            base64_encrypted_message (str) : зашифрованное сообщение в Base64.
            quantum_key_str (str)          : строковое представление квантового ключа.

        Возвращает:
            str : расшифрованное сообщение (или строка с пометкой об ошибке при неверных данных).
        """
        try:
    ...
    def simulate_qkd_bb84(self, num_bits, eve_active=False):
        """
        Симулировать протокол BB84 квантового распределения ключей.

        Алиса готовит случайные кубиты в случайных базисах. Боб измеряет их
        в случайных базисах. Если Ева активна, она перехватывает и повторно
        пересылает каждый кубит, внося ошибки (QBER ~25%).

        Параметры:
            num_bits (int)    : количество передаваемых кубитов.
            eve_active (bool) : True -- Ева перехватывает канал.

        Возвращает:
            tuple: (qber, key_alice, key_bob)
                qber (float)     -- квантовый уровень битовых ошибок в процентах.
                key_alice (str)  -- согласованный ключ Алисы (строка из '0' и '1').
                key_bob (str)    -- согласованный ключ Боба (строка из '0' и '1').
        """
        ...
        for i in range(num_bits):
            ...
            if bob_basis == 1:
                qc.h(0)
            qc.measure(0, 0)
            res_bob = self.simulator.run(transpile(qc, self.simulator), shots=1).result().get_counts()
    ...
    def classical_message_exchange(self, shared_key, alice_message_generator, bob_message_generator):
        """
        Провести классический обмен сообщениями через защищённый квантовым ключом канал.

        Каждое сообщение зашифровывается, затем Ева пытается расшифровать его
        своим (неверным) ключом -- получает мусор. Получатель расшифровывает
        сообщение корректным ключом.

        Параметры:
            shared_key (str)                  : согласованный квантовый ключ.
            alice_message_generator (callable): генератор (или функция-генератор) сообщений Алисы.
            bob_message_generator (callable)  : генератор (или функция-генератор) сообщений Боба.
        """
        print("\n--- Классический обмен сообщениями через безопасный канал ---")

        print(f"\n--- Сообщения от {self.alice_name} ---")
        for msg in alice_message_generator():
        ...
        for msg in bob_message_generator():
    ...
    def find_secure_channel(self, num_bits, eve_active=True, max_attempts=100, qber_threshold=10):
        """
        Итеративно запускать BB84 до получения канала с QBER ниже порога.

        Если Ева активна, QBER обычно ~25%, что превышает порог, и канал
        не устанавливается (подтверждается безопасность протокола).

        Параметры:
            num_bits (int)       : количество кубитов на попытку.
            eve_active (bool)    : True -- Ева перехватывает канал.
            max_attempts (int)   : максимальное количество попыток.
            qber_threshold (int) : допустимый QBER в процентах (по умолчанию 10%).

        Возвращает:
            str : согласованный ключ Алисы при успехе, пустая строка при неудаче.
        """
        print(f"\n--- Попытка установить безопасный канал (количество бит: {num_bits}, "
              f"{self.eve_name}: {'Активна' if eve_active else 'Выключена'}, "
              f"Порог QBER: <{qber_threshold}%) ---")
        attempt = 0
        qber = 100
        shared_key = ""
        ...
        while qber >= qber_threshold and attempt < max_attempts:
            ...
            if qber < qber_threshold:
                print(f"[OK] Безопасный канал установлен на попытке {attempt}! "
                      f"QBER = {qber:.1f}%. Ключи используются для связи.\n")
                shared_key = key_alice
                break
            else:
                print(f"[!] Обнаружены ошибки ({qber:.1f}%) или QBER выше допустимого порога "
                      f"(<{qber_threshold}%). Попытка установить канал снова.\n")

        if qber >= qber_threshold:
            print(f"[ВНИМАНИЕ] Не удалось установить безопасный канал после "
                  f"{max_attempts} попыток. QBER = {qber:.1f}%")
            return ""
        return shared_key
    ...
    def display_2d_harmonic_oscillator_animation(self, save_video=False,
                                                  gif_filename='2D_harmonic_oscillator.gif'):
        """
        Построить и (опционально) сохранить 2D-анимацию квантового гармонического осциллятора.

        Волновая функция является суперпозицией нескольких состояний (nx, ny).
        Анимируется временная эволюция вероятностной плотности |Psi(x, y, t)|^2.

        Параметры:
            save_video (bool)     : True -- сохранить анимацию в GIF-файл.
            gif_filename (str)    : имя выходного GIF-файла.
        """
        ...
        im = ax.imshow(np.zeros((grid_points, grid_points)), origin='lower',
                       extent=[x_lim[0], x_lim[1], y_lim[0], y_lim[1]],
                       cmap='viridis', vmin=0, vmax=0.1)
        fig.colorbar(im, ax=ax, label='|Psi(x, y, t)|^2')
    ...
    def run_demonstration(self, alice_name="Алиса", bob_name="Боб", eve_name="Ева",
                          alice_msg_gen=None, bob_msg_gen=None):
        """
        Запустить полную демонстрацию квантовой коммуникации.

        Сценарий:
            1. Алиса и Боб устанавливают QKD-ключ (с Евой или без).
            2. Классический обмен зашифрованными сообщениями.
            3. (Опционально) Хранение пароля Боба в квантовой памяти (QuantumMemoryServer).
            4. Попытка QKD с активной Евой -- демонстрация обнаружения атаки.

        Параметры:
            alice_name (str)       : имя первой стороны.
            bob_name (str)         : имя второй стороны.
            eve_name (str)         : имя перехватчика.
            alice_msg_gen (callable): генератор сообщений Алисы (None -- используются сообщения по умолчанию).
            bob_msg_gen (callable)  : генератор сообщений Боба (None -- используются сообщения по умолчанию).
        """
        ...
        if not shared_key_AB:
            ...
            if self.use_quantum_memory:
                ...
                if eve_retrieved_encrypted_base64:
                    ...
                    print(f"{self.eve_name}: Пытается расшифровать своим ключом: '{eve_wrong_key[:20]}...\n"
                          f"{self.eve_name} получает: '{eve_decrypted_attempt}' (Мусор)")
                ...
                if alice_retrieved_encrypted_base64:
                    alice_decrypted_attempt = self.decrypt_message(
                        alice_retrieved_encrypted_base64, shared_key_AB)
                    print(f"{self.alice_name}: Получила данные {self.bob_name} "
                          f"и расшифровала своим ключом: '{alice_decrypted_attempt}'")

            print("\n" + "*"*20 + f" ПОВТОРНАЯ ПОПЫТКА QKD C {self.eve_name.upper()} " + "*"*20)
            shared_key_with_eve = self.find_secure_channel(
            ...
            if not shared_key_with_eve:
                ...
                print(f"[ПРЕДУПРЕЖДЕНИЕ] QKD-ключ был установлен, несмотря на {self.eve_name}. "
                      f"(Это не должно произойти часто)")


class QuantumMemoryServer:
    """
    Сервер квантовой памяти на основе квантовых гармонических осцилляторов (кудиты).

    Данные клиентов кодируются побайтово в уровни Фока квантовых осцилляторов
    с помощью QuTiP и хранятся в массиве слотов памяти. Запись и считывание
    данных осуществляются через методы store_client_data / retrieve_client_data.

    Параметры:
        num_qudit_levels (int)      : количество уровней каждого кудита (основание системы счисления).
        memory_capacity_qudits (int): общее число доступных слотов-осцилляторов.
    """

    def __init__(self, num_qudit_levels=10, memory_capacity_qudits=100):
        if num_qudit_levels < 2:
            raise ValueError("Количество уровней кудита должно быть не менее 2.")
        if num_qudit_levels > 256:
            print("Предупреждение: num_qudit_levels > 256 может привести к неэффективному кодированию.")
        ...
        print(f"\nQuantumMemoryServer запущен с {memory_capacity_qudits} осцилляторами "
              f"(каждый до {num_qudit_levels} уровней).\n")
    ...
    def _encode_bytes_to_qudits(self, data_bytes):
        """
        Закодировать байтовую последовательность в последовательность кудитов.

        Каждый байт (0..255) представляется несколькими цифрами в системе счисления
        с основанием num_qudit_levels.

        Параметры:
            data_bytes (bytes) : входные данные.

        Возвращает:
            list[int] : список уровней кудитов.
        """
        qudit_sequence = []
    ...
    def _decode_qudits_to_bytes(self, qudit_sequence, original_bytes_len):
        """
        Декодировать последовательность кудитов обратно в байты.

        Параметры:
            qudit_sequence (list[int])  : список уровней кудитов.
            original_bytes_len (int)    : ожидаемое число байт на выходе.
        ...
        if len(qudit_sequence) % num_qudits_per_byte != 0:
            print(f"Предупреждение: длина последовательности кудитов ({len(qudit_sequence)}) "
                  f"не кратна кудитам на байт ({num_qudits_per_byte}). Возможно повреждение данных.")
    ...
    def store_client_data(self, client_id, encrypted_base64_message):
        """
        Сохранить зашифрованные данные клиента в квантовую память.

        Параметры:
            client_id (str)                  : идентификатор клиента.
            encrypted_base64_message (str)   : зашифрованные данные в формате Base64.

        Возвращает:
            bool : True при успехе, False при нехватке памяти или ошибке кодирования.
        """
        ...
        if len(qudit_sequence) > self.memory_capacity:
            print(f"[ОШИБКА Сервера] Недостаточно памяти для клиента '{client_id}'. "
                  f"Требуется {len(qudit_sequence)} qudits, доступно {self.memory_capacity}.")
            return False

        start_idx = 0
        for i, qudit in enumerate(qudit_sequence):
            if qudit < 0 or qudit >= self.num_qudit_levels:
                print(f"[ОШИБКА Сервера] Недопустимое значение кудита: {qudit}. "
                      f"Должно быть в диапазоне [0, {self.num_qudit_levels - 1}].")
                return False
        ...
        print(f"Сервер: Данные клиента '{client_id}' ({len(qudit_sequence)} qudits) "
              f"успешно сохранены в квантовую память.")
        return True

    def retrieve_client_data(self, client_id):
        """
        Извлечь данные клиента из квантовой памяти.

        Параметры:
            client_id (str) : идентификатор клиента.

        Возвращает:
            str | None : зашифрованное Base64-сообщение или None, если данные не найдены.
        """
...
if __name__ == '__main__':
    ...
    qsim_instance.display_2d_harmonic_oscillator_animation(
        save_video=True, gif_filename='2D_harmonic_oscillator.gif')
